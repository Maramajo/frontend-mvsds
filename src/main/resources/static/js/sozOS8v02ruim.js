...
async function fetchContent(endpoint, method = 'GET', body = null) {
  ...
  */
      ...
      // --- PARTE 1 corrigida ---
      response = await fetch(`http://maramajo.ddns.net:32000${endpoint}`, {
        method: method,
        headers: {
                                ...
                                if (externalUrl.includes('buscaxma')) {
                                  ...
                                  if (externalUrl.includes('buscaxme')) {
                                    ...
                                    if (externalUrl.includes('KMP1101P')) {
                                      ...
                                      if (externalUrl.includes('cartaoEN')) {
                                        newUrl = externalUrl.replace("http://192.168.0.13:3000/CICS/CWBA/KMP1101P", "/xml");
                                        fetchContent(newUrl, 'POST', body);

                                      } else
                                        if (externalUrl.includes('cartaoDE')) {
                                          newUrl = externalUrl.replace("http://192.168.0.13:3000/CICS/CWBA/KMP1101P", "/xml");
                                          fetchContent(newUrl, 'POST', body);
                                        }
  ...
  function renderContent(path, lang) {
    ...
    // 🔥 ALTERAÇÃO AQUI: reinicializa tooltips sempre que trocar view via routeFunction
    initTooltips();
  ...
  function changeLang(lang) {
    ...
    if (endpointMap[path]) {
      ...
      const redirectMap = {
        ...
        '/cartaoEN': 'http://192.168.0.13:3000/CICS/CWBA/KMP1101P',
  ...
  // 🔥 NOVO: função única para (re)ligar os tooltips
